...
export async function generateAISummary(
  ...
  const systemPrompt = type === 'my'
    ? '你是一个纯文本摘要引擎。用户会发送一份技能注册表的 markdown 文本。你必须根据其中的数据生成一段客观的概览摘要（3-5 句话），只总结技能数量、分类方向和能力覆盖。规则：只输出纯中文段落文字，不要使用 markdown 格式，不要给建议，不要提问，不要模拟对话，不要输出"你可以..."之类的引导语。'
    : '你是一个纯文本摘要引擎。用户会发送一份外部技能来源注册表的 markdown 文本。你必须根据其中的数据生成一段客观的概览摘要（2-3 句话），只总结来源数量、类型和接入状态。规则：只输出纯中文段落文字，不要使用 markdown 格式，不要给建议，不要提问，不要模拟对话，不要输出"你可以..."之类的引导语。'
