...
function renderVendorOverviewList(items: VendorSummary[]): string {
  return items
    .map(v => `- **${v.name}**：${v.fetchedSkills.length} 个技能`)
    .join('\n')
}

export function renderMySkillsMarkdown(data: RegistryData): string {
...
export function renderOtherSkillsMarkdown(data: RegistryData): string {
...
### 来源概览
${renderVendorOverviewList(data.vendors)}
...
async function main(): Promise<void> {
  ...
  // 翻译 vendor 技能描述为中文（需要 AI_BASE_URL + AI_API_KEY + AI_MODEL 环境变量 + --summarize 标志）
  ...
  if (options.outputMy)
    writeFileSync(options.outputMy, renderMySkillsMarkdown(data), 'utf8')

  if (options.outputOther)
    writeFileSync(options.outputOther, renderOtherSkillsMarkdown(data), 'utf8')

  if (!options.outputMy && !options.outputOther)
    console.log(renderMySkillsMarkdown(data))
}
